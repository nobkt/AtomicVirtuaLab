...
from AtomicVirtuaLab.siesta import mk_siesta_input_optimize,mk_siesta_input_nvt,mk_siesta_input_scf_withEfield
...
for mol in mollist:
    ...
    for basis in ['SZ','SZP','DZ','DZP']:
        ...
        os.chdir(str(basis))
        for cutoff in [50.0,100.0,200.0]:
            os.makedirs('cutoff_'+str(cutoff),exist_ok=True)
            os.chdir('cutoff_'+str(cutoff))
            # no efield
            os.makedirs('e0',exist_ok=True)
            os.chdir('e0')            
            mk_siesta_input_scf_withEfield(xyz,'VDW',basis,cutoff,[1,1,1],g.siesta_pot,ex=0.0,ey=0.0,ez=0.0,SolutionMethod='diagon',MaxSCFIterations=2000,spin='non-polarized')
            os.chdir('../')
            # ex+
            os.makedirs('ex+',exist_ok=True)
            os.chdir('ex+')            
            mk_siesta_input_scf_withEfield(xyz,'VDW',basis,cutoff,[1,1,1],g.siesta_pot,ex=epsilon,ey=0.0,ez=0.0,SolutionMethod='diagon',MaxSCFIterations=2000,spin='non-polarized')
            os.chdir('../')
            # ex-
            os.makedirs('ex-',exist_ok=True)
            os.chdir('ex-')            
            ...
            os.chdir('ey+')            
            mk_siesta_input_scf_withEfield(xyz,'VDW',basis,cutoff,[1,1,1],g.siesta_pot,ex=0.0,ey=epsilon,ez=0.0,SolutionMethod='diagon',MaxSCFIterations=2000,spin='non-polarized')
            os.chdir('../')
            # ey-
            os.makedirs('ey-',exist_ok=True)
            os.chdir('ey-')            
            ...
            os.chdir('ez+')            
            mk_siesta_input_scf_withEfield(xyz,'VDW',basis,cutoff,[1,1,1],g.siesta_pot,ex=0.0,ey=0.0,ez=epsilon,SolutionMethod='diagon',MaxSCFIterations=2000,spin='non-polarized')
            os.chdir('../')
            # ez-
            os.makedirs('ez-',exist_ok=True)
            os.chdir('ez-')            
